...
def load_profile(bins):
	...
	DPATH = "/uufs/chpc.utah.edu/common/home/astro/zheng"\
			"/pfli/LyaCGMProfile/SimData/AstroRe/"
	logM = [10, 10.5, 11, 11.5, 12]
	logf = [-1.5, -1.25, -1.0, -0.75, -0.5]
	logcHI = [0, 0.5, 1.0, 1.5, 1.2]
	xtrain = []
	ytrain = []
	for i in range(len(logM)):
		for j in range(len(logf)):
			for k in range(len(logcHI)):
				photon_name = DPATH + "ModNFW_logM=%.1f_logfesc=%.2f"\
						"_logcHI=%.2f_CEN_photons.txt"%(
						logM[i], logf[j], logcHI[k])
				if check_file(photon_name) == False:
					continue
				fname = DPATH + "ModNFW_logM=%.1f_logfesc=%.2f"\
						"_logcHI=%.2f_CEN_param.txt"%(
						logM[i], logf[j], logcHI[k])
				gas = lp.LyaGasModel_DST1(fname)
				...
				xtrain.append(np.array([logM[i], logf[j], logcHI[k]]))
				ytrain.append(sb[1])
...
def training_selection():
	...
	xm, ym = training_xy()
#	set 1
	pt = [[10.5, -0.75, 0.0], [11.5, -0.75, 0.0], 
		  [10.0, -0.75, 0.5], [11.0, -0.75, 0.5], [12.0, -0.75, 0.5],
		  [10.5, -0.75, 1.0], [11.5, -0.75, 1.0]]
#	set 2
#	pt = [[10.0, -0.75, 0.0], [10.5, -0.75, 0.0], [11.5, -0.75, 0.0], [12.0, -0.75, 0.0],
#		  [10.0, -0.75, 0.5], [11.0, -0.75, 0.5], [12.0, -0.75, 0.5],
#		  [10.5, -0.75, 1.0], [11.5, -0.75, 1.0]]
#	set 3
#	pt = [[10.0, -0.75, 0.0], [10.5, -0.75, 0.0], [11.5, -0.75, 0.0], 
#		  [11.5, -0.75, 0.0], [12.0, -0.75, 0.0],
#		  [10.0, -0.75, 0.5], [11.0, -0.75, 0.5], [12.0, -0.75, 0.5],
#		  [10.5, -0.75, 1.0], [11.0, -0.75, 1.0], [11.5, -0.75, 1.0]]
...
def predicting_selection():
	...
	xm, ym = training_xy()
#	set 1
#	pt = [[10.0, -0.75, 0.0], [11.0, -0.75, 0.0], [12.0, -0.75, 0.0], 
#		  [10.5, -0.75, 0.5], [11.5, -0.75, 0.5],
#		  [11.0, -0.75, 1.0]]
#	Success
	pt = [[11.5, -0.75, 0.5], [11.0, -0.75, 1.0], [11.0, -0.75, 0.0]]
#	Failure
#	pt = [[10.0, -0.75, 0.0],  [12.0, -0.75, 0.0], [10.5, -0.75, 0.5]]
#	set 2
#	pt = [[11.0, -0.75, 0.0], 
#		  [10.5, -0.75, 0.5], [11.5, -0.75, 0.5],
#		  [11.0, -0.75, 1.0]]
#	set 3
#	pt = [[10.5, -0.75, 0.5], [11.5, -0.75, 0.5]]
...
#	theta = [2]
	...
	for i in range(len(xp)):
		label = r"$\log M=%.1f,\log c_{\rm HI}=%.2f$"%(xp[i][0], xp[i][2])
...
def main_lnprob():
	...
	ax.set_ylabel(r"$-2\ln\ Prob$", fontsize = 10)
